...
def get_address(db: Session, skip: int = 0, limit: int = 50):
    return db.query(address_model.CustomerAddress).offset(skip).limit(limit).all()

def get_address_by_id(db: Session, address_id: int):
    return db.query(address_model.CustomerAddress).filter(address_model.CustomerAddress.address_id==address_id).first()


# create address
def create_address(db: Session, address: address_schema.CreateCustomerAddress):
    new_address = address_model.CustomerAddress(
...
#update address info
def update_address(db: Session, address_id: int, address: address_schema.UpdateCustomerAddress):
    address_data = db.query(address_model.CustomerAddress).filter(address_model.CustomerAddress.address_id == address_id).first()
